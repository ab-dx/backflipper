...
class RENDER:
    ...
    def render_sticc_boi_from_frame(self, frame):
        """Creates the window that displays the sticcboi
    ...
    def mark_from_frame(self, frame):
        """makes variable point map and joints from frame (from csv list)
        ...
        self.joints = set()
        counter = 0
        for p,k in zip(range(0,len(frame),3), config['ragdoll']['unique']):
            frame[p] *= self.SCALE
            frame[p+1] *= self.SCALE
            frame[p+2] *= self.SCALE
            # print("POINT MAP START")
            # print(frame[p], frame[p+1], frame[p+2])
            # print("POINT MAP END")
            self.point_map[k] = Vector3(frame[p],frame[p+1],frame[p+2])
            counter += 1
